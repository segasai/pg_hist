...
CREATE or replace FUNCTION pg_hist_1d_w(query text, nbins int8[], 
     minimums float8[], maximums float8[],
     x1 out int , hist out double precision ) RETURNS setof record
    AS 'MODULE_PATHNAME'
    LANGUAGE C;
    
CREATE or replace FUNCTION pg_hist_2d_w(query text, nbins int8[], 
     minimums float8[], maximums float8[],
     x1 out int , x2 out int, hist out double precision ) RETURNS setof record
    AS 'MODULE_PATHNAME'
    LANGUAGE C;

CREATE or replace FUNCTION pg_hist_3d_w(query text, nbins int8[],
     minimums float8[], maximums float8[],
     x1 out int , x2 out int, x3 out int, hist out double precision ) RETURNS setof record
    AS 'MODULE_PATHNAME'
    LANGUAGE C;